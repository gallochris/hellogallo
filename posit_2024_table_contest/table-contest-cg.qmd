---
title: "Posit Table Contest 2024"
author: "Chris Gallo"
date: "2024-05-20"
format: 
  html:
...
## 2024 ACC Baseball Standings and Tournament Seeding

This Quarto document contains a table built in R using the [{gt}](https://gt.rstudio.com/) and [{gtExtras}](https://jthomasmock.github.io/gtExtras/) packages for the [2024 Posit Table Contest](https://posit.co/blog/announcing-the-2024-table-contest/). The source of the data is from the {[baseballr](https://billpetti.github.io/baseballr/)} package and [ncaa.com](https://www.ncaa.com/rankings/baseball/d1/rpi). All data manipulation done using the {[dplyr](https://dplyr.tidyverse.org/)}, {[tidyr](https://tidyr.tidyverse.org/)}, and {[rvest](https://rvest.tidyverse.org/)} packages.
...
```{r acc-team-ids, warning=FALSE, message=FALSE}
...
```{r results-function, warning=FALSE, message=FALSE}
...
```{r diff-data-fetch, warning=FALSE, message=FALSE}
...
Next, we're going to write a function to scrape the RPI table from [ncaa.com](https://www.ncaa.com/rankings/baseball/d1/rpi). This is helpful to grab the team's overall record, plus it adds context to how each conference team ranks nationally.

```{r rpi-function, warning=FALSE, message=FALSE}
...
```{r pool-play, warning=FALSE, message=FALSE}
...
Now, we're ready to generate the table. We're going to modify the theme and use several useful functions from the {[gtExtras](https://jthomasmock.github.io/gtExtras/)} package.

This also uses the {[cbbplotR](https://cbbplotr.aweatherman.com/articles/getting_started.html)} package to add in logos of the teams.

First, we're going to generate a custom GT theme.

```{r gt-custom-theme, warning=FALSE, message=FALSE}
...
Here is the code used to generate the static HTML table. Features of the table include:

-   merging and stacking columns: [`gtExtras::gt_merge_stack`](https://jthomasmock.github.io/gtExtras/reference/gt_merge_stack.html)

-   function to add a `+` sign to positive run differentials: [`gt::fmt`](https://gt.rstudio.com/reference/fmt.html)

-   lots of styling of text to reference columns: [`gt::tab_style`](https://gt.rstudio.com/reference/tab_style.html)

-   color scheme for good/bad differentials and win percentages: [`gtExtras::gt_hulk_col_numeric`](https://jthomasmock.github.io/gtExtras/reference/gt_hulk_col_numeric.html)

```{r gt-table, warning=FALSE, message=FALSE}
library(cbbplotR)
